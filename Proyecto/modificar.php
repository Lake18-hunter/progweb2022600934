...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['tipo']) && $_POST['tipo'] === 'alumno') {
        $stmt = $conexion->prepare("UPDATE alumnos SET grado=?, grupo=?, edad=?, turno=? WHERE id=?");
        $stmt->bind_param("ssisi", $_POST['grado'], $_POST['grupo'], $_POST['edad'], $_POST['turno'], $_POST['id']);
        $stmt->execute();
        $stmt->close();
        ...
        $stmt = $conexion->prepare("UPDATE profesores SET grado=?, grupo=?, turno=?, materia=? WHERE id=?");
        $stmt->bind_param("ssssi", $_POST['grado'], $_POST['grupo'], $_POST['turno'], $_POST['materia'], $_POST['id']);
        $stmt->execute();
        $stmt->close();
...
<body class="container py-4">
  ...
  <h2 class="mb-4">Modificar Usuarios</h2>
  ...
  <?php while ($row = $alumnos->fetch_assoc()): ?>
    <form method="POST" class="border p-3 mb-3">
      ...
      <div class="row">
        <div class="col-md-2"><input name="usuario" value="<?= $row['username'] ?>" class="form-control" required></div>
        <div class="col-md-2"><input name="password" value="<?= $row['password'] ?>" class="form-control" required></div>
        <div class="col-md-1">
        ...
        <div class="col-md-1"><input name="edad" value="<?= $row['edad'] ?>" class="form-control" required></div>
        <div class="col-md-2">
        ...
        <div class="col-md-1"><button type="submit" class="btn btn-warning">Guardar</button></div>
        <div class="col-md-2"><a href="modificar.php?eliminar=<?= $row['id'] ?>&tipo=alumno" class="btn btn-outline-danger">Eliminar</a></div>
      </div>
    </form>
  <?php endwhile; ?>
  ...
  <?php while ($row = $profesores->fetch_assoc()): ?>
    <form method="POST" class="border p-3 mb-3">
      ...
      <div class="row">
        ...
        <div class="col-md-1"><button type="submit" class="btn btn-warning">Guardar</button></div>
        <div class="col-md-1"><a href="modificar.php?eliminar=<?= $row['id'] ?>&tipo=profesor" class="btn btn-outline-danger">Eliminar</a></div>
      </div>
    </form>
  <?php endwhile; ?>
</body>
